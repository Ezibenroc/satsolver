...
void parser_result(SPEF ext_formula) {
    ...
    std::shared_ptr<satsolver::Formula> formula = ext_formula->reduce_to_formula(&name_to_variable);
    ...
    satsolver::Affectation sat_solution = satsolver::solve(&*formula); // May raise a satsolver::Conflict
    if (VERBOSE)
...
int main (int argc, char *argv[]) {
    CommandLineParser cli_parser(argc, argv, std::unordered_set<std::string>({"-print-sat"}), "[-print-sat] [<filename>]");
